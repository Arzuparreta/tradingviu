...
- **9h (done) — Pivot Points:** `packages/pivot-points` (pure engine — groups
  bars into D/W/M periods and derives each period's levels from the prior
  period's range for 5 methods: standard, fibonacci, camarilla, woodie, demark);
  `POST /api/pivot-points`; and a **Pivots** toggle on `ChartPage` (method +
  period selectors) drawing the latest period's levels as price lines + a levels
  panel. See `docs/SLICE-9.md`.
- Per-candle footprint cells (bid/ask split) — **deferred**, needs trade-level data

### Slice 10 — Ecosystem

- **10a (done) — Personal access tokens + public `/v1` + OpenAPI:** API-key
  primitives in `@tv/auth` (generate/hash/parse/verify, `tvk_…`), an
  `access_tokens` table (migration `0009`), `/api/access-tokens` management,
  an `apiKeyContext` middleware mirroring `tenantContext`, a public `/v1`
...
- **10d (done) — Public WebSocket streaming API:** `/v1/ws` authenticates
  personal access tokens via `?api_key=...`, `Authorization: Bearer`, or
  `X-API-Key`, requires `read`, rate-limits upgrades per token, and reuses the
  existing live `bar` / `quote` / `book` stream protocol. See `docs/SLICE-10.md`.
- Later — webhook out, plugin SDK, Pine v6 compatibility.
